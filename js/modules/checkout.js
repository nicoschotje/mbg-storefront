...
import { computeUsdtDue } from './crypto-pricing.js?v=20260708-usdt';
...
function refreshDelivery(host) {
  ...
  if (payBox) {
    ...
    // the total (hence the exact USDT amount due) just changed, flag it so the
    // re-render warns them to re-check and re-upload — never a silent mismatch
    // between the amount they sent and the amount now owed.
    if (_selectedPay === 'usdt' &&
        payBox.querySelector('#receiptFile')?.files?.length &&
        _lastRenderedTotal != null && Math.abs(total - _lastRenderedTotal) > 0.001) {
      _usdtReceiptStale = true;
    }
    renderPayInfo(payBox, _selectedPay, total);
  }
